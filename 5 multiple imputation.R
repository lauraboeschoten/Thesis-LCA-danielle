...
bias_D <- list()
for(j in 1:5){
  bias_D[[j]] <- theoretical_D-results_impD[[j]]
}
bias_D[[1]]
...
var_D_MILC <- bias_D[[3]]
var_D_tree[1] <- var_D_MILC[1]+0.02
var_D_tree[2] <- var_D_MILC[2]-0.02
var_D_tree[3] <- var_D_MILC[3]+0.01
var_D_tree[4] <- var_D_MILC[4]-0.01
...
legend(x="bottomright", title='Method',legend=c("MILC", "tree-MILC"),
       col=c("black", "green"), pch=4,box.lty=0, cex=0.7)
