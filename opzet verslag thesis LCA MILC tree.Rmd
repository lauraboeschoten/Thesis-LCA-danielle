...
probs3 <- list(matrix(c(0.9,0.025,0.025,
                        0.025,0.9,0.025,
                        0.025,0.025,0.9 ), ncol=3,   byrow=TRUE), # Y1
               matrix(c(0.9,0.025,0.025,
                        0.025,0.9,0.025,
                        0.025,0.025,0.9 ), ncol=3, byrow=TRUE), # Y2
               matrix(c(0.90,0.025,0.025,
                        0.025,0.9,0.025,
                        0.025,0.025,0.9 ), ncol=3, byrow=TRUE), #Y3
               matrix(c(0.025,0.025,0.90,
                        0.025,0.9,0.025,
                        0.90,0.025,0.025 ), ncol=3, byrow=TRUE))#Y4
set.seed(125)
...
### Comments
Still to be added are the categorical covariates. They will be added to the datasets with the polca function as if they were another indicator variable. 

### Analysis
Needed: a dataframe with selection and classification error (four variables, and the trueclass to check)
```{r}
summary((as.factor(df1[,5]))) #how many per class  
check2 <- poLCA(formula=(cbind(Y1,Y2,Y3,Y4)~1), nrep = 10,data = df1, nclass=2) #0.33 en 0.67 (label problem!) en later 0.53, 0.47
...
check3 <- poLCA(formula=(cbind(Y1,Y2,Y3,Y4)~1),data = df1, nclass=3) #0.2, 0.3, 0.4
